...
The worker checks the exact support queue via the fixed production snapshot helper, the legacy Drive intake folder, GitHub/Vercel deployment parity, and bounded Vercel log queries. On any actionable condition it dispatches `monitor-alert.yml`, which creates a deduplicated GitHub Issue with fixed reason codes. When `YUTAKASA_MONITOR_OWNER_EMAIL_ENABLED=true`, that workflow also emails `181wyc@gmail.com` about new error or owner-decision Issues created on or after `YUTAKASA_MONITOR_OWNER_EMAIL_CUTOFF`, or an older open Issue whose reason recurs after enablement. The email subject and first lines explain the issue and next action in Japanese; fixed codes remain in the private Issue. Routine queued-work reasons do not generate an error email. The separate scheduled job retries an unconfirmed email every 20 minutes. The Issue number is the Resend idempotency key, and bot comments record the first attempt and verified delivery. After 23 hours without a verified result, the workflow fails closed and does not risk a second send. Existing Issues older than the cutoff are not mailed solely because the scheduled job ran. Current technical anomalies also dispatch `ai-repair.yml` for an isolated investigation and draft PR or issue. No ticket body or Drive filename is sent in monitor email or Issue; the separate owner-only technical escalation email contains a bounded excerpt of the exact latest user report after a service-role lookup. A Vercel token, Drive API key, or snapshot failure results in a nonzero exit and no healthy verdict. The support GET may recover stale ticket locks and add recovery work logs, so do not run it concurrently with the desktop automation during cutover.
...
Apply `repair-release-ledger.sql`, then `support-automation-reply.sql`, `ticket-repair-bridge.sql`, `ticket-reply-draft.sql`, `ticket-clarification.sql`, and `ticket-clarification-notice.sql`; verify service-role-only RPC grants and run `bash scripts/automation/test-support-automation-reply-sql.sh`. The bridge handles only technical tickets without attachments or owner-decision terms. An atomic locked handoff creates a random work UUID and moves the ticket to `awaiting_repair`. A new customer message invalidates the old work. After a verified release, the scheduled review may save a private Terra-generated draft for the administrator. The Vercel flag `TICKET_REPLY_DRAFTS_ENABLED` defaults off; enable it only after the draft table and RPC migration has been applied and checked in production. The checked send API also rejects draft use while the flag is off. The review still moves the ticket to manual review and sends no customer message. The draft migration revokes the service role's repair-completion reply RPC permission until ticket-specific before/after evidence and an approval route are implemented. A separate clarification RPC sends one fixed request for missing screen, action, error, and time details, only for an exact, short first technical report without attachments or earlier replies. The outbox migration reserves a fixed app-link email in the same transaction and suppresses known synthetic identities. The email never includes the ticket body. Both the Railway worker and Vercel API require `TICKET_CLARIFICATION_ENABLED=true` and `TICKET_CLARIFICATION_NOTICE_ENABLED=true`; GitHub needs `YUTAKASA_TICKET_CLARIFICATION_NOTICE_ENABLED=true`, the Resend secret, and its reconcile schedule. Keep all flags off until the migration, isolated production smoke, provider acceptance, and scheduled fallback are verified. Unsupported categories remain for a person to review.

The manual `ticket-clarification-smoke.yml` runs only on main and additionally requires repository variable `YUTAKASA_CLARIFICATION_SMOKE_ENABLED=true`. Apply `ticket-clarification-notice.sql`, then reapply `ticket-clarification-smoke-cleanup.sql` and verify its service-role-only EXECUTE grant. Run `bash scripts/automation/test-ticket-clarification-smoke-sql.sh` locally or in CI. Before writing, the one-shot job rejects any old `yutakasa-auto-smoke+*@example.invalid` identity or related ticket, chat, or OTP row and probes both deployed Vercel clarification flags. It then creates only an isolated `yutakasa-auto-smoke+UUID@example.invalid` subscriber and an exact `使えない` technical ticket, claims it by explicit ID, asks the fixed in-app question, checks the direct RPC retry is idempotent and the API retry is rejected, and confirms one admin message, one clarification ledger row, one suppressed notice row, two expected logs, and no unrelated dependents. No OpenAI call, customer address, notification email, or payment is used. The guarded cleanup RPC atomically validates the marker, lock token, exact ticket shape, and suppressed notice before deleting the synthetic subscriber and ticket; an always-running rescue step uses a mode-0600 job-local UUID file if the main process is interrupted. The rescue step remains available if the repository smoke variable is switched off during the job. After one successful run and zero-residue readback, set `YUTAKASA_CLARIFICATION_SMOKE_ENABLED=false` and return both Vercel clarification flags to `false`; verify both values. Keep both Railway clarification flags `false` during this smoke; enable them only after the smoke succeeds and the production schedule can be observed. A skipped or failed job is not evidence of activation.
...
The repair-release observer has a separate Railway fallback, disabled by default. First verify the `ai-repair-observe.yml` manual `mode=observe` workflow on main, the capped-project functional smoke, and its guarded cleanup. Then set the GitHub repository variable `YUTAKASA_REPAIR_OBSERVER_RAILWAY_FALLBACK_ENABLED=true` so the GitHub schedule job skips while explicit `mode=observe` dispatch remains available; only after that set the same variable on the Railway monitor service. When enabled, Railway reads at most five metadata-only active releases and their current UTC ten-minute-slot observations. An `observing` release is due if it has no observation in the slot; a `pending_merge` release is due only when its matching PR has merged, closed, or exceeded the 30-minute pending deadline. The monitor checks the observer's active and current-slot completed GitHub runs, then dispatches fixed `{ref:"main",inputs:{mode:"observe"}}` after releasing its lease. The GitHub job and release ledger provide additional concurrency and `(pr_number,cron_slot)` duplicate guards. A due release records `repair_observation_due`; a failed lookup or dispatch exits nonzero and alerts with fixed `repair_observer_dispatch_failed`. GitHub dispatch acceptance is not a verified release. To disable fallback, first unset the Railway variable, then restore the GitHub schedule by unsetting its variable. No customer text or ticket identifier enters this path.
