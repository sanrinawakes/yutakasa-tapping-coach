...
Apply `repair-release-ledger.sql`, then `support-automation-reply.sql`, then `ticket-repair-bridge.sql`; verify service-role-only RPC grants and run `bash scripts/automation/test-support-automation-reply-sql.sh`. The bridge handles only technical tickets without attachments or owner-decision terms. An atomic locked handoff creates a random work UUID and moves the ticket to `awaiting_repair`. A new customer message invalidates the old work. Unsupported categories remain for a person to review.
