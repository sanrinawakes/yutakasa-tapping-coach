...
BEGIN;
LOCK TABLE public.yutakasa_ticket_completion_proofs IN SHARE ROW EXCLUSIVE MODE;
...
DO $$ BEGIN
  IF EXISTS(SELECT 1 FROM public.yutakasa_ticket_completion_proofs p
    LEFT JOIN public.yutakasa_ticket_completion_notices n
      ON n.work_id=p.work_id AND n.message_id=p.used_message_id
    WHERE p.used_message_id IS NOT NULL AND n.work_id IS NULL) THEN
...
BEGIN
  ...
  IF NOT FOUND OR v_ticket.user_email IS NULL
    OR char_length(v_ticket.user_email)>254
    OR v_ticket.user_email !~
      '^[^[:space:]@,;<>"[:cntrl:]]+@[^[:space:]@,;<>"[:cntrl:]]+\.[^[:space:]@,;<>"[:cntrl:]]+$'
    OR v_ticket.subject IS NULL
    OR char_length(v_ticket.subject) NOT BETWEEN 1 AND 120
    OR v_ticket.subject ~ '[[:cntrl:]]' THEN
