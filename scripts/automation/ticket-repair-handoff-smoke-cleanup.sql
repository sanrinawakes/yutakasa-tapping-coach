-- Install before running the one-shot production handoff probe.
...
CREATE OR REPLACE FUNCTION public.cleanup_yutakasa_ticket_handoff_smoke(
  p_run_id UUID,
  p_account_id UUID,
...
DECLARE
  ...
  v_rows BIGINT;
BEGIN
  IF p_run_id IS NULL OR p_work_id IS NULL OR p_lock_token IS NULL THEN
  ...
  IF v_ticket.id IS NOT NULL THEN
    ...
    SELECT count(*),
      ...
      INTO v_log_count,v_claim_count,v_handoff_count
...
REVOKE ALL ON FUNCTION public.cleanup_yutakasa_ticket_handoff_smoke(
  UUID,UUID,TIMESTAMPTZ,UUID,TIMESTAMPTZ,UUID,UUID)
  FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.cleanup_yutakasa_ticket_handoff_smoke(
  UUID,UUID,TIMESTAMPTZ,UUID,TIMESTAMPTZ,UUID,UUID) TO service_role;
