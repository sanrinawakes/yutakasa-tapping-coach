...
function fakeDrive({ existing = false, existingFile, uploadStatus = 200, onCall = () => {} } = {}) {
  const expectedMetadata = {
    name: "豊かさBOT_対応結果_20260917_release_123456.pdf",
    appProperties: {
  ...
  return async (url, init) => {
    ...
    if (parsed.pathname === "/drive/v3/files") {
      assert.match(parsed.searchParams.get("q"), /name = '豊かさBOT_対応結果_20260917_release_123456.pdf'/u);
    ...
    if (parsed.pathname === "/upload/drive/v3/files") {
      ...
      assert.ok(init.body.includes(Buffer.from("豊かさBOT_対応結果_20260917")));
