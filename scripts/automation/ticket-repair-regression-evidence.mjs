...
export async function runRegressionEvidence({
  ...
  try {
    ...
    // Run the identical new regression test against the unmodified old source.
    await copyFile(headTest, baseTest);
    const before = await runVitest(base, context.testFile, path.join(temp, "before.json"), true);
    const after = await runVitest(head, context.testFile, path.join(temp, "after.json"), false);
