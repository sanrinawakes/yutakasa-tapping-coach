...
2026-09-18 JST以降は、返信が必要な案件に限って最新の質問・報告の
最大240文字の抜粋を掲載する。一般的なメールアドレスと長い数字列は
伏せ、添付と作業ログの原文は含めない。この抜粋はサービスロールだけが
読める日報の不変スナップショットにも保存される。旧日付の2宛先分には
質問・報告本文を含めない。
本文の冒頭に、運営者の判断と返信が必要な件数、自動対応が止まった件数、
問い合わせの種類、質問・報告の抜粋、管理画面で行うことを日本語で示す。
